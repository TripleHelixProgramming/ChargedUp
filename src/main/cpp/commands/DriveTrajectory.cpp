...
void DriveTrajectory::Execute() {
  ...
  auto vx = state.vx + meters_per_second_t{controllerX.Calculate(currentPose.X().value())};
  auto vy = state.vy + meters_per_second_t{controllerY.Calculate(currentPose.Y().value())};
  auto omega = state.omega + radians_per_second_t{controllerRotation.Calculate(currentPose.Rotation().Radians().value())};

  SmartDashboard::PutNumber("Target x: ", state.pose.X().value());
  SmartDashboard::PutNumber("Target y: ", state.pose.Y().value());
  SmartDashboard::PutNumber("Target rotation: ", state.pose.Rotation().Radians().value());

  SmartDashboard::PutNumber("Real x: ", currentPose.X().value());
  SmartDashboard::PutNumber("Real y: ", currentPose.Y().value());
  SmartDashboard::PutNumber("Real rotation: ", currentPose.Rotation().Radians().value());
