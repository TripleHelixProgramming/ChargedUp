...
Trajectory::State Trajectory::State::Interpolate(const Trajectory::State& other,
  ...
  return Trajectory::State{
      newT, pose.Exp(pose.Log(other.pose) * scale),
      ChassisSpeeds{
          (other.velocity.vx - velocity.vx) * scale + velocity.vx,
...
void from_json(const json& j, Trajectory::State& state) {
  ...
  state.velocity = ChassisSpeeds{
      meters_per_second_t{j.at("velocityX").get<double>()},
