...
Trajectory TrajectoryGenerator::Generate(Pose2d start, Pose2d end) {
  ...
  int N = 100;
  double linearVnorm = std::hypot((end.X() - start.X()).value(), 
                                  (end.Y() - start.X()).value(), 
                                  (end.Rotation() - start.Rotation()).Radians().value());
  ChassisSpeeds linearVHat{meters_per_second_t{(end.X() - start.X()).value() / linearVnorm}, 
                           meters_per_second_t{(end.Y() - start.Y()).value() / linearVnorm}, 
                           radians_per_second_t{(end.Rotation() - start.Rotation()).Radians().value() / linearVnorm}};
  for (int index = 0; index < N; ++index) {
    double scale = (double)index / (N - 1);
    poses.emplace_back((end.X() - start.X()) * scale + start.X(), 
  ...
  for (int index = 0; index < N - 1; ++index) {
    ...
    meters_per_second_t maxAllowableVelocityX = sgn(currentVelocityHat.vx.value()) * 
        (2 * maxAccelerationX * delta.dx + currentVelocityNorm * currentVelocityHat.vx);
    meters_per_second_t maxAllowableVelocityY = ;
    radians_per_second_t maxAllowableRotationalVelocity = ;
    
    v_norm = std::min({maxAllowableVelocityX / v_hats[index].vx,
  ...
  for (size_t index = 1; index < poses.size(); ++index) {
    ...
    if (std::abs((poses[index].X() - poses[index-1].X()).value()) > 1e-6) {
      ...
      v = v_norms[index - 1] * (v_hats[index - 1].vx + v_hats[index].vx).value() / 2.0;
    } else if (std::abs((poses[index].Y() - poses[index-1].Y()).value()) > 1e-6) {
      ds = (poses[index].Y() - poses[index - 1].Y()).value();
      v = v_norms[index - 1] * (v_hats[index - 1].vy + v_hats[index].vy).value() / 2.0;
    } else {
      ds = (poses[index].Rotation() - poses[index - 1].Rotation()).Radians().value();
      v = v_norms[index - 1] * (v_hats[index - 1].omega + v_hats[index].omega).value() / 2.0;
    }
    trajectoryStates.emplace_back(second_t{ds / v} + trajectoryStates[index - 1].t, poses[index], 
