...
MaxAccelerationConstraint::MaxAccelerationConstraint(
...
double MaxAccelerationConstraint::MaxVelocityNormForward(
  ...
  if (endVelocityHat.vx.value() != 0.0) {
    if (vx_hat > 0.0) {
      double v = std::sqrt(
          2.0 * dx * m_maxAccelerationX +
          std::pow(startVelocityNorm * startVelocityHat.vx.value(), 2));
  ...
  if (endVelocityHat.vy.value() != 0.0) {
    if (vy_hat > 0.0) {
      double v = std::sqrt(
          2.0 * dy * m_maxAccelerationY +
          std::pow(startVelocityNorm * startVelocityHat.vy.value(), 2));
  ...
  if (endVelocityHat.omega.value() != 0.0) {
    if (omega_hat > 0.0) {
      double v = std::sqrt(
          ...
          std::pow(startVelocityNorm * startVelocityHat.omega.value(), 2));
