...
std::optional<EstimatedRobotPose> PhotonPoseEstimator::Update() {
  ...
  constexpr auto tagCornerToObjectPoint = [](Translation3d corner, Pose3d tagPose) {
  ...
  SmartDashboard::PutNumber("SQPNP/Rotation/Yaw", pose.Rotation().Z().convert<degree>().value());
  SmartDashboard::PutNumber("SQPNP/Rotation/Pitch", pose.Rotation().Y().convert<degree>().value());
  SmartDashboard::PutNumber("SQPNP/Rotation/Roll", pose.Rotation().X().convert<degree>().value());
  SmartDashboard::PutNumber("SQPNP/Time", std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count());
