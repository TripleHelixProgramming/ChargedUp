...
def animate_arm_trajectory(fps, dt, states, arm, reference, save):
    ...
    def animate(i):
        arm_line.set_data(
            [0, arm.l * cos(states[i][0])], [0, arm.l * sin(states[i][0])]
        )
        ref_line.set_data(
            [0, arm.l * cos(reference[0])], [0, arm.l * sin(reference[0])]
        )
        return [arm_line]

    anim = FuncAnimation(
        fig, animate, frames=len(states), interval=int(dt * 1000), blit=True
    )
    ...
    if save:
        ...
        anim.save(
            os.path.join(
                "src/main/python/animations", "{}.mp4".format("One-DOF Arm Simulation")
            ),
            writer="ffmpeg",
            dpi=250,
            fps=fps,
